...
(defmacro +lazy-load-symbol (symbol func)
...
(defmacro +make-async-shell-cmd (cmd &optional mode)
...
CMD can be a a string or a list of strings.
    ...
    (+make-async-shell-cmd (\"make all\" \"make test\") 'makefile-mode)
...
MODE can be a 'symbol (cons), a symbol or a variable that points to a symbol.
    ...
    (+make-async-shell-cmd \"make all\" 'c-mode)
    (+make-async-shell-cmd \"make all\" c-mode)
    (+make-async-shell-cmd \"make all\" mode) ;; MODE points to 'c-mode.

These are all valid and results in a function named +shell-c-mode-make-all.

If MODE is nil, something like:

    (+make-async-shell-cmd \"make all\")

will result in a function named +make-all."
  (let ((commands
         (if (consp cmd)
             cmd
           (list cmd))))
...
(defmacro +make-terminal-dot-app-command (cmd)
  "Creates a function that runs a terminal CMD in Terminal.app."
  (let ((name (intern (concat "+terminal-app-"
...
(defun +desktop-p ()
  ...
  (or
   ...
   (member (system-name) '("jnguyen-mac2"))
   (> (+monitor-width) 4400)))
...
(defun +open-shell ()
  ...
  (cond
   ...
   (t
    (message "Implement `+open-shell' for this OS!"))))
...
(defun +open-terminal ()
  ...
  (cond
   ...
   (t
    (message "Implement `+open-terminal' for this OS!"))))
...
do a search for the string from projet root to mimic that functionality."
  ...
  (cond
   ...
   (t
    (message "Implement `+find-references' for this OS!"))))
...
(defun +project-c-style ()
  "Determine c style of project."
  (condition-case nil
      (let ((proot (projectile-project-root)))
        (cond
         ((and buffer-file-name (string-match "mymod" buffer-file-name))
          "gnu")
...
(defun +post-to-reviewboard (reviewboard-id)
  "Helper function to post to reviewboard."
  (interactive "sReviewboard Id: (None for New Review) ")
  (let ((server "http://reviewboard")
        (repository "gitfusion-mobile-app")
        (tracking-branch "origin/perforce/developer"))
    (cond
     ((string-empty-p reviewboard-id)
      (message "Creating new CR.")
      ;; rbt post -o --server http://reviewboard --repository gitfusion-mobile-app --tracking-branch=origin/perforce/developer
      (start-process-shell-command
       "rbt" "*rbt*"
       (format
        "rbt post -o --server %s --repository %s --tracking-branch=%s"
        server
        repository
        tracking-branch)))
     (:else
      (let ((cr-id (string-to-number reviewboard-id)))
        ;; rbt post -r 159372 -o --server http://reviewboard --repository gitfusion-mobile-app --tracking-branch=origin/perforce/developer
        (if (equal cr-id 0)
            (message "Reviewboard id is not in a valid format.")
          (message (format "Updating CR %d" cr-id))
          (start-process-shell-command
           "rbt" "*rbt*"
           (format
            "rbt post -r %d -o --server %s --repository %s --tracking-branch=%s"
            cr-id
            server
            repository
            tracking-branch))))))))

(defalias 'rbt-post '+post-to-reviewboard)
(defalias 'reviewboard-post '+post-to-reviewboard)
