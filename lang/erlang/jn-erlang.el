...
(use-package erlang
  ...
  ("\\.xrl\\'" . erlang-mode)
  :init
  (defun +setup-inferior-erlang ()
    "Launch erlang shell in background"
    (when (null (inferior-erlang-running-p))
      (save-selected-window
        (inferior-erlang)
        (quit-window))))
  (add-hook 'erlang-mode-hook #'+setup-inferior-erlang)
  :config
...
(use-package company-distel
  ...
  (add-hook 'erlang-mode-hook
            (lambda ()
              (+company-push-backend 'company-distel t))))

;;;###autoload
(defun +erlang-mode ()
  "Bootstrap `jn-erlang'."
  ;; (setq auto-mode-alist (rassq-delete-all #'+erlang-mode auto-mode-alist))
